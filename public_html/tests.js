...
//Оптимизировал http get, свел практически на нет обращения к серверу, 700 точек запросом, результат:
//1) В кэшере нету данных, время на все 420 - 550 ms
//2) После повторного запроса, данные из кэша: 56 - 68 ms вместе с функциями qunit, self time: 13  ms (и 80 процентов этого времени, это превращение данных в нужный вид)
//3) Тоесть конкетенация и заталкивание всех данных в нужные массивы: 11 ms
//4) Соответственно время на то чтобы достать эти данные из базы 2 ms
//Теперь 12000 точек:
//1) В кэшере нету данных: 1300 - 1500 ms
//2) После повторного запроса: 210 - 225 ms
//3) Сбор данных, конкетенация: 185 - 190 ms
//4) Достаются данныe из базы 24 - 28 ms
//Для построения графика 12000 это слишком много, ему для хорошого разрешения достаточно 800-1200, сделал про 12000 только ради интереса.
//
//
//--
//С уважением,
